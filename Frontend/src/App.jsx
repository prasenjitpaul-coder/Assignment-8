    ...
    <>
      <div className="bg-amber-200 h-screen flex  items-center justify-center relative">
        <div className="bg-amber-400 h-4/6 w-3xl rounded-md overflow-hidden shadow-2xl shadow-gray-800">
          <div className="bg-amber-900 h-18 rounded flex items-center justify-between">
            <h3 className="text-4xl font-extrabold ml-5 ">To-dO-LIST</h3>
            <div>
              <input
                className="bg-white rounded-l-2xl pl-1"
                value={query}
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    handelSearch();
              ...
              />
              <button
                className="mr-5 rounded-r-2xl bg-white border pl-1 pr-1 cursor-pointer"
                onClick={handelSearch}> Enter 
                </button>
            </div>
          </div>
          <div className="h-120 mx-2  mt-2 ">
            {(query && Array.isArray(result) ? result : data).map((task) => (
              <div
                className="flex bg-orange-500 mb-2  justify-between  rounded shadow-lg shadow-amber-800"
                key={task._id}
              >
                <div className="px-2 ">
                  {Editid === task._id ? (
                    <input
                      className={`text-2xl  focus:outline-none border-none  placeholder-gray-900 font-bold ${task.complete ? "line-green " : ""}`}
                      value={Edit}
                      spellCheck="false"
                      autoCorrect="off"
                      autoCapitalize="off"
                    ...
                    <p
                      className={`text-2xl font-bold ${task.complete ? "line-green" : ""}`}
                    >
                      {task.task}
                    </p>
                  )}
                ...
                <div className="flex cursor-pointer  items-center gap-2 mr-2">
                  <div
                    onClick={() => toggleline(task._id, task.complete)}
                    className="bg-amber-500  px-2 pt-1 rounded-3xl    shadow-lg shadow-amber-800"
                  >
          ...
          <div className="flex ">
            ...
            >
              ...
              <input
                className="bg-white py-2.5 rounded-2xl  shadow-md shadow-gray-700 pl-1 pr-8 mx-3"
                value={task}
              ...
              <button
                ...
                className="bg-orange-500 cursor-pointer  text-gray-900 text-3xl mx-auto flex px-5 py-2.5 rounded-2xl shadow-md shadow-gray-700"
              >
                Create Task
              </button>
            </form>
